...
use git2::{
    AnnotatedCommit, Cred, CredentialType, FetchOptions, PushOptions, RemoteCallbacks, Repository,
};
use std::path::Path;
...
pub fn auth_ssh(_: &str, usr_url: Option<&str>, _: CredentialType) -> Result<Cred, git2::Error> {
...
pub fn push(repo: &Repository) -> Result<(), error::Error> {
    let remotes = repo.remotes()?;
    if remotes.is_empty() {
        Err(internal!("repo has no remotes. cannot push"))?;
    }
    let mut remote = repo.find_remote(remotes.get(0).unwrap())?;
    let branch = current_branch(repo)?;
    let mut push_opt = PushOptions::default();
    let mut callbacks = RemoteCallbacks::default();
    callbacks.credentials(auth_ssh);
    push_opt.remote_callbacks(callbacks);
    remote.push(&[&format!("refs/heads/{}", &branch)], Some(&mut push_opt))?;
    Ok(())
}
...
pub fn fetch(repo: &Repository) -> Result<AnnotatedCommit, error::Error> {
    let remotes = repo.remotes()?;
    if remotes.is_empty() {
        Err(internal!("repo has no remotes. cannot fetch"))?;
    }
    let mut remote = repo.find_remote(remotes.get(0).unwrap())?;
    let branch = current_branch(repo)?;
